...
public class PLVECCommodityPopupView {
    ...
    // <editor-fold defaultstate="collapsed" desc="商品操作">
    //新增/上架商品
    public void add(PolyvProductContentBean productContentBean, boolean isNewly) {
    ...
    //更新商品
    public void update(PolyvProductContentBean productContentBean) {
        if (popupWindow == null || !popupWindow.isShowing()) {
            return;
        }
    ...
    //上/下移商品
    public void move(PolyvProductMoveEvent productMoveEvent) {
        if (popupWindow == null || !popupWindow.isShowing()) {
            return;
        }
        ...
        PolyvProductContentBean handleContent = productMoveEvent.getContent().get(0);//操作的商品
        PolyvProductContentBean contentBean = productMoveEvent.getContent().get(1);
        commodityAdapter.move(handleContent, contentBean);
    }
    // </editor-fold>

    // <editor-fold defaultstate="collapsed" desc="设置、添加商品数据">
    ...
    private List<PLVBaseViewData> toViewDataList(PolyvCommodityVO commodityVO) {
        ...
        if (commodityVO != null && commodityVO.getData() != null) {
            List<PolyvProductContentBean> contentsBeanList = commodityVO.getData().getContent();
            if (contentsBeanList != null) {
                for (PolyvProductContentBean contentsBean : contentsBeanList) {
    ...
    private void updateNoMoreDataStatus(List<PLVBaseViewData> viewDataList) {
        if (commodityVO != null && viewDataList.size() < PLVLiveRoomManager.GET_COMMODITY_COUNT) {//每次加载20条数据，<20表示没有更多数据了
            isNoMoreData = true;
            if (smoothRefreshLy != null) {
                smoothRefreshLy.refreshComplete();
    ...
    // <editor-fold defaultstate="collapsed" desc="更新商品数view">
