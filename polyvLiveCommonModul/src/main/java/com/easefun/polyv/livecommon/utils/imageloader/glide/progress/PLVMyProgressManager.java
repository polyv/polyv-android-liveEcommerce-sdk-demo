package com.easefun.polyv.livecommon.utils.imageloader.glide.progress;
...
public class PLVMyProgressManager {
    ...
    private static Map<String, Map<Object, PLVOnProgressListener>> listenersMap =
            Collections.synchronizedMap(new HashMap<String, Map<Object, PLVOnProgressListener>>());
...
//            listener.onProgress(url, false, 1, 0, 0);//slide list, progress yet, no call
    ...
    public static void removeListener(String moduleTag, Object urlTag) {
        Map<Object, PLVOnProgressListener> objectList = getProgressListenerList(moduleTag);
        if (objectList != null) {
            for (Object tag : objectList.keySet()) {
                if (tag.equals(urlTag)) {
                    objectList.get(tag).transListener(null);
                    objectList.remove(tag);
                    break;
                }
            }
        }
    ...
    public static Map<Object, PLVOnProgressListener> getProgressListenerList(String moduleTag) {
    ...
    public static Map<String, Map<Object, PLVOnProgressListener>> getListenersMap() {
        return listenersMap;
    }
    ...
    public static PLVOnProgressListener getProgressListener(String moduleTag, Object urlTag) {
        ...
        if (objectList != null) {
            for (Object tag : objectList.keySet()) {
                if (tag.equals(urlTag)) {
                    return objectList.get(tag).getTransListener();
                }
            }
        }
        return null;
